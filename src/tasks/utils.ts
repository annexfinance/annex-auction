...
export async function getAnnexAuctionContract({
  ethers,
  deployments,
}: HardhatRuntimeEnvironment): Promise<Contract> {
  const authenticatorDeployment = await deployments.get("AnnexAuction");
...
export async function getWETH9Address(
  ...
  if (chainId == 4) {
    ...
    weth9Address = "0x049eCB1a93bD4608B9e7ECe911baA2e412BE5b9B";
  }
  return weth9Address;
}
