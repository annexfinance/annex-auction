...
import { getAnnexAuctionContract } from "./utils";
...
const placeManyOrders: () => void = () => {
  task("placeManyOrders", "Allows to place many orders for testing purposes")
    ...
    .setAction(async (taskArgs, hardhatRuntime) => {
      ...
      const annexAuction = await getAnnexAuctionContract(hardhatRuntime);
      if (annexAuction.address != "0xC5992c0e0A3267C7F75493D0F717201E26BE35f7") {
        throw new Error("Use the script only on rinkeby");
      }
      const auctionData = await annexAuction.callStatic.auctionData(
        taskArgs.auctionId,
      );
      ...
      console.log("Using AnnexAuction deployed to:", annexAuction.address);
      ...
      if (totalSellingAmountInAtoms.gt(allowance)) {
        ...
        const tx = await auctioningToken
          .connect(caller)
          .approve(annexAuction.address, totalSellingAmountInAtoms);
        await tx.wait();
      ...
      for (let i = 0; i < taskArgs.nrOfOrders / orderBlockSize; i += 1) {
        ...
        const tx = await annexAuction.connect(caller).placeSellOrders(
          taskArgs.auctionId,
          minBuyAmounts,
